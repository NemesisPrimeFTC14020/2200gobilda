...
@Autonomous(name = "Lv32QuarryBlue", group = "Linear OpMode")
public class Level32Blue extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (lastSkystone == null) {
            updatedRecognitions = tfod.getUpdatedRecognitions();
            if (updatedRecognitions != null) {
                for (Recognition recognition : updatedRecognitions) {
                    if (recognition.getLabel().equals("Skystone")) {
                        lastSkystone = recognition;
                    }
                }
            }
        }
            angle = lastSkystone.estimateAngleToObject(AngleUnit.DEGREES);

            x = 54 * Math.tan(angle);
           // bp.encDriveMecanumin(1, x, y, this);
        }
    }
